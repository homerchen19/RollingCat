var menuState = {
   ...
   jumpoption:function(){
     if(!option_d){
        game.add.tween(option).to({y:-option.height/2},2000,Phaser.Easing.Exponential.In,true,0,0,false);
     }
     else{
        game.add.tween(option).to({y:0},2000,Phaser.Easing.Exponential.In,true,0,0,false);
     }
     option_d = !option_d; 
   }
